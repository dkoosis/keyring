...
// TestNew_RejectsRelativeKeychain pins that WithKeychain must reject a
...
// TestGet_KeychainArgReachesFindGenericPassword pins that when
...
// TestGet_LockedKeychainStderrContainsPhraseIsUnreadable pins that a
...
// TestWrite_KeychainArgReachesAddGenericPassword pins the write
...
// TestSet_RejectsNonASCII pins that `security find-generic-password -w`
...
// TestSet_ValueErrorDoesNotLeakBytes pins that printableASCIIOnly's error
...
// TestSet_RejectsEmptyAccount pins that New rejects an empty/whitespace
...
// indeterminate contract: the write (`security -i`) and the
...
// TestSetIfAbsent_KeychainArgReachesAddGenericPassword pins honoring
...
func TestLiveKeychain(t *testing.T) {
	...
	// (a >128-byte OAuth access token). 100-char values masked the bug.
